...
def main(
    ...
    image = torchvision.io.read_image(
        input_path, mode=torchvision.io.ImageReadMode.GRAY
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run inference with a trained wavelet module"
    )
    parser.add_argument(
        "--config", help="Path to the module config file", default="params/config.yaml"
    )
    parser.add_argument(
        "--state", help="Path to the module state file", default="params/state.pt"
    )
    parser.add_argument("--input", required=True, help="Path to the input image")
    parser.add_argument(
        "--grayscale",
        help="Path to save the input image, but grayscale (png)",
        default=None,
    )
    parser.add_argument(
        "--output", required=True, help="Path to save the output image (png)"
    )
